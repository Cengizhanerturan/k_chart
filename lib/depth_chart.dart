    ...
    return GestureDetector(
      ...
      child: CustomPaint(
        ...
        painter: DepthChartPainter(
          ...
          widget.chartColors,
          widget.depthBuyColor,
          widget.depthSellColor,
          widget.selectFillColor,
...
class DepthChartPainter extends CustomPainter {
  ...
  DepthChartPainter(
    ...
    mBuyPathPaint ??= Paint()
      ..isAntiAlias = true
      ..color = (mBuyPathColor ?? depthBuyColor.withOpacity(0.2));
    mSellPathPaint ??= Paint()
      ..isAntiAlias = true
      ..color = (mSellPathColor ?? depthSellColor.withOpacity(0.2));
    mBuyPath ??= Path();
    mSellPath ??= Path();
    init();
  }
  ...
  void drawBuy(Canvas canvas) {
    ...
    double x;
    double y;
    for (var i = 0; i < mBuyData!.length; i++) {
      if (i == 0) {
  ...
  void drawSell(Canvas canvas) {
    ...
    double x;
    double y;
    for (var i = 0; i < mSellData!.length; i++) {
      if (i == 0) {
  ...
  void drawText(Canvas canvas) {
    ...
    var startTP = getTextPainter(startText);
    startTP.layout();
    startTP.paint(canvas, Offset(0, getBottomTextY(startTP.height)));

    var centerPrice = (mBuyData!.last.price + mSellData!.first.price) / 2;

    var center = centerPrice.toStringAsFixed(fixedLength!);
    var centerTP = getTextPainter(center);
    centerTP.layout();
    centerTP.paint(
        canvas,
        Offset(
    ...
    var endTP = getTextPainter(endText);
    endTP.layout();
    endTP.paint(
    ...
    var leftHalfTP = getTextPainter(leftHalfText);
    leftHalfTP.layout();
    leftHalfTP.paint(
        canvas,
    ...
    var rightHalfText = ((mSellData!.last.price + centerPrice) / 2)
        .toStringAsFixed(fixedLength!);
    var rightHalfTP = getTextPainter(rightHalfText);
    rightHalfTP.layout();
    rightHalfTP.paint(
        canvas,
  ...
  void drawSelectView(Canvas canvas, int index, bool isLeft) {
    var entity = isLeft ? mBuyData![index] : mSellData![index];
    var dx = isLeft ? getBuyX(index) : getSellX(index);
    ...
    //画底部
    var priceTP = getTextPainter(entity.price.toStringAsFixed(fixedLength!));
    priceTP.layout();
    double left;
    ...
    var bottomRect = Rect.fromLTRB(left - 3, mDrawHeight + 3,
    ...
    //画左边
    var amountTP = getTextPainter(entity.vol.toStringAsFixed(fixedLength!));
    amountTP.layout();
    var y = getY(entity.vol);
    double rightRectTop;
    ...
    var rightRect = Rect.fromLTRB(mWidth - amountTP.width - 6, rightRectTop - 3,
        mWidth, rightRectTop + amountTP.height + 3);
