...
//    }
    ...
    mLineFillShader ??= LinearGradient(
      ...
      colors: [lineFillColor, lineFillInsideColor],
    ...
    canvas.drawPath(mLinePath!,
        mLinePaint..strokeWidth = (mLineStrokeWidth / scaleX).clamp(1.5, 2.0));
    mLinePath!.reset();
  }

  void drawMaLine(CandleEntity lastPoint, CandleEntity curPoint, Canvas canvas,
      double lastX, double curX) {
    for (var i = 0; i < (curPoint.maValueList?.length ?? 0); i++) {
      if (i == 3) {
        break;
      }
  ...
  void drawBollLine(CandleEntity lastPoint, CandleEntity curPoint,
    ...
    if (lastPoint.up != 0) {
      drawLine(lastPoint.up, curPoint.up, canvas, lastX, curX, ma10Color);
    }
    if (lastPoint.mb != 0) {
      drawLine(lastPoint.mb, curPoint.mb, canvas, lastX, curX, ma5Color);
    }
    if (lastPoint.dn != 0) {
      drawLine(lastPoint.dn, curPoint.dn, canvas, lastX, curX, ma30Color);
    }
  }
  ...
  void drawVerticalText(canvas, textStyle, int gridRows) {
    var rowSpace = chartRect.height / gridRows;
    for (var i = 0; i <= gridRows; ++i) {
      var value = (gridRows - i) * rowSpace / scaleY + minValue;
      var span = TextSpan(text: '${format(value)}', style: textStyle);
      var tp = TextPainter(text: span, textDirection: TextDirection.ltr);
      tp.layout();
